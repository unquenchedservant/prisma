...
class RubiksPocketCubeRandomScrambler(override val scramblerInfo: ScramblerInfo, minScrambleLength: Int, generatingSet: Array<String>?) : Scrambler {
    private val solver: RubiksPocketCubeSolver
    private val random: Random
    override val nextScramble: Scramble
        get() = Scramble(
                scramblerInfo.scramblerId,
                solver.generate(
                        solver.getRandomState(random)!!))
    ...
    init {
        solver = RubiksPocketCubeSolver(minScrambleLength, generatingSet!!)
        random = Random()
    }
}