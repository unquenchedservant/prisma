...
class RubiksCubeLUScrambler(override val scramblerInfo: ScramblerInfo) : Scrambler {
    private val random: Random
    override val nextScramble: Scramble
        get() {
            val mirror = HashMap<String?, String>()
            mirror["U"] = "U'"
            mirror["U2"] = "U2"
            mirror["U'"] = "U"
            mirror["R"] = "L'"
            mirror["R2"] = "L2"
            mirror["R'"] = "L"
            val sequence = RubiksCubeRUSolver.generate(
                    RubiksCubeRUSolver.getRandomState(random))
            for (i in sequence.indices) {
                sequence[i] = mirror[sequence[i]]
            }
            return Scramble(
