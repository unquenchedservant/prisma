...
class MegaminxRandomScrambler(scramblerInfo: ScramblerInfo) : Scrambler {
    override val scramblerInfo: ScramblerInfo
    private val random: Random
    override val nextScramble: Scramble
        get() {
            val sequence = arrayOfNulls<String>(77)
            for (i in 0..6) {
                for (j in 0..4) {
                    sequence[11 * i + 2 * j] = if (random.nextInt(2) == 0) "R++" else "R--"
                    sequence[11 * i + 2 * j + 1] = if (random.nextInt(2) == 0) "D++" else "D--"
                }
                sequence[11 * i + 10] = if (sequence[11 * i + 9] == "D++") "U" else "U'"
            }
            return Scramble(
