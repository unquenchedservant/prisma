...
class StackmatGraphPanel : JPanel() {
    ...
    public override fun paintComponent(g: Graphics) {
        ...
        if (data != null) {
            for (i in 0 until data!!.size - 1) {
                g2.drawLine((width.toDouble() / data!!.size.toDouble() * i.toDouble()).toInt(), (-(data!![i].toDouble() * (height.toDouble() / (Byte.MAX_VALUE - Byte.MIN_VALUE).toDouble())) + height.toDouble() / 2).toInt(), (width.toDouble() / data!!.size.toDouble() * (i + 1)) as Int, (-(data!![i + 1].toDouble() * (height.toDouble() / (Byte.MAX_VALUE - Byte.MIN_VALUE).toDouble())) + height.toDouble() / 2).toInt())
            }
        }
    }
