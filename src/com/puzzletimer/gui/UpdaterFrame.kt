...
class UpdaterFrame(url: String, version: String) {
    var updateUrl: String
    var progress: JProgressBar
    var progressLabel: JLabel
    var version: String
    fun downloadLatestVersion() {
        val t = Thread {
            try {
                ...
                if (HttpURLConnection.HTTP_OK == hConnection.responseCode) {
                    hConnection.inputStream.use { `in` ->
                        BufferedOutputStream(
                                FileOutputStream(File(UpdaterFrame::class.java.protectionDomain.codeSource.location.toURI().path)
                                )).use { out ->
                            val filesize = hConnection.contentLength
                            ...
                            while (`in`.read(buffer).also { numRead = it } != -1) {
                                ...
                                numWritten += numRead.toLong()
                                println(numWritten.toDouble() / filesize.toDouble())
                                progress.value = numWritten.toInt()
                            }
                            if (filesize.toLong() != numWritten) println("Wrote $numWritten bytes, should have been $filesize") else println("Downloaded successfully.")
                            out.close()
                            `in`.close()
                            try {
                                val update = Runtime.getRuntime().exec("java -jar " + File(UpdaterFrame::class.java.protectionDomain.codeSource.location.toURI().path))
                            } catch (e: IOException) {
                                e.printStackTrace()
    ...
    init {
        ...
        updateUrl = url
        frame.preferredSize = Dimension(300, 80)
        frame.size = Dimension(300, 80)
        frame.title = "Updater"
