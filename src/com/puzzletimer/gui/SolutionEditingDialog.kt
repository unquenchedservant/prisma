...
class SolutionEditingDialog(
        owner: JFrame?,
        modal: Boolean,
        solution: Solution?,
        listener: SolutionEditingDialogListener,
        configurationManager: ConfigurationManager) : JDialog(owner, modal) {
    open class SolutionEditingDialogListener {
        open fun solutionEdited(solution: Solution?) {}
    }

    private var textFieldStart: JTextField? = null
    private var textFieldTime: JTextField? = null
    private var comboBoxPenalty: JComboBox<*>? = null
    private var textFieldScramble: JTextField? = null
    private var textFieldComment: JTextField? = null
    ...
    private fun createComponents() {
        ...
        // labelStart
        add(JLabel(_("solution_editing.start")))

        // textFieldStart
        textFieldStart = JTextField()
        textFieldStart!!.isEditable = false
        textFieldStart!!.isFocusable = false
        add(textFieldStart)

        // labelTime
        add(JLabel(_("solution_editing.time")))
        ...
        // labelPenalty
        add(JLabel(_("solution_editing.penalty")))

        // comboBoxPenalty
        comboBoxPenalty = JComboBox<Any?>()
        add(comboBoxPenalty)

        // labelScramble
        add(JLabel(_("solution_editing.scramble")))

        // textFieldScramble
        textFieldScramble = JTextField()
        textFieldScramble!!.isEditable = false
        textFieldScramble!!.isFocusable = false
        add(textFieldScramble)

        // labelComment
        add(JLabel(_("solution_editing.comment")))

        //textFieldComment
        textFieldComment = JTextField()
        add(textFieldComment)

        // buttonOk
        buttonOk = JButton(_("solution_editing.ok"))
        add(buttonOk, "tag ok, span 2, split")

        // buttonCancel
        buttonCancel = JButton(_("solution_editing.cancel"))
        add(buttonCancel, "tag cancel")
    }

    init {
        title = _("solution_editing.solution_editor")
        minimumSize = Dimension(480, 200)
        createComponents()
        pack()

        // start
        val dateFormat = DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.MEDIUM)
        textFieldStart!!.text = dateFormat.format(solution!!.timing.start)

        // time
        textFieldTime!!.text = formatMinutes(solution.timing.elapsedTime, configurationManager.getConfiguration("TIMER-PRECISION")!!, false)
        addWindowFocusListener(object : WindowAdapter() {
            override fun windowGainedFocus(e: WindowEvent) {
                // text selection
                textFieldTime!!.selectionStart = 0
                textFieldTime!!.selectionEnd = Int.MAX_VALUE
        ...
        // penalty
        comboBoxPenalty!!.addItem("")
        comboBoxPenalty!!.addItem("+2")
        comboBoxPenalty!!.addItem("DNF")
        comboBoxPenalty!!.selectedItem = solution.penalty

        // scramble
        textFieldScramble!!.text = solution.scramble.rawSequence
        textFieldScramble!!.caretPosition = 0

        // comment
        textFieldComment!!.text = solution.comment
        ...
        buttonOk!!.addActionListener { event: ActionEvent? ->
            ...
            val time = parseTime(
                    textFieldTime!!.text)
            val timing = Timing(
                    solution.timing.start,
                    Date(solution.timing.start.time + time))

            // penalty
            val penalty = comboBoxPenalty!!.selectedItem as String
            val comment = textFieldComment!!.text
