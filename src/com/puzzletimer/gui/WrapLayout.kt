...
class WrapLayout(private val xGap: Int, private val yGap: Int) : LayoutManager {
    ...
    override fun minimumLayoutSize(target: Container): Dimension {
        ...
        val maxWidth = target.width
        val nComponents = target.componentCount
        var x = insets.left
        var y = insets.top
        for (i in 0 until nComponents) {
            val component = target.getComponent(i)
            val w = component.preferredSize.getWidth().toInt()
            val h = component.preferredSize.getHeight().toInt()
    ...
    override fun layoutContainer(target: Container) {
        synchronized(target.treeLock) {
            ...
            for (i in 0 until nComponents) {
                ...
                val h = component.preferredSize.getHeight().toInt()
                if (x + w + insets.right > maxWidth) {
                    val d = (maxWidth - (x - xGap) - insets.right) / 2
                    for (j in firstComponent until i) {
                        val c = target.getComponent(j)
                        c.setLocation(c.x + d, c.y)
                    }
                    firstComponent = i
                    x = insets.left
                    y += h + yGap
            ...
            val d = (maxWidth - (x - xGap) - insets.right) / 2
            for (j in firstComponent until nComponents) {
                val c = target.getComponent(j)
                c.setLocation(c.x + d, c.y)
            }
        }
