...
class TimeLabel(private var text: String) : JComponent() {
    private var timerPrecisionId: String? = null
    fun setText(text: String) {
        this.text = text
        repaint()
    }

    fun setTimerPrecision(timerPrecisionId: String?) {
    ...
    override fun paint(g: Graphics) {
        ...
        var textWidth = g2.fontMetrics.stringWidth("00:00.00")
        if (timerPrecisionId == "CENTISECONDS") {
            textWidth = g2.fontMetrics.stringWidth("00:00.00")
        } else if (timerPrecisionId == "MILLISECONDS") {
            textWidth = g2.fontMetrics.stringWidth("00:00.000")
        }
        val fontSize = font.size * width / textWidth
        g2.font = font.deriveFont(Math.min(fontSize, height).toFloat())
        val bounds = g2.fontMetrics.getStringBounds(text, g2)
        g2.drawString(
                text,
                ((width - bounds.width) / 2 - bounds.x).toInt(),
                ((height - bounds.height) / 2 - bounds.y).toInt())
    }
}