...
class RubiksPocketCube : Puzzle {
    override val puzzleInfo: PuzzleInfo
        get() = PuzzleInfo("2x2x2-CUBE")
    ...
    override fun getScrambledPuzzleMesh(colorScheme: ColorScheme, sequence: Array<String>): Mesh? {
        ...
        val planeL = Plane(Vector3(-0.0, 0, 0), Vector3(-1, 0, 0))
        val planeR = Plane(Vector3(0.0, 0, 0), Vector3(1, 0, 0))
        val planeD = Plane(Vector3(0, -0.0, 0), Vector3(0, -1, 0))
        val planeU = Plane(Vector3(0, 0.0, 0), Vector3(0, 1, 0))
        val planeF = Plane(Vector3(0, 0, -0.0), Vector3(0, 0, -1))
        val planeB = Plane(Vector3(0, 0, 0.0), Vector3(0, 0, 1))
        mesh = mesh
                .cut(planeR, 0.0)
                .cut(planeU, 0.0)
                .cut(planeF, 0.0)
